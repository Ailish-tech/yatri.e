 ...
 *
 * @remarks
 * - Ensure that the API endpoint (`http://192.168.1.156:3000/api/generateItinerary`) is accessible.
 ...
 *
 * @remarks
 * - Ensure that the API endpoint (`http://192.168.1.156:3000/api/justchat`) is correctly configured.
...
export const generateChatAnswers = async (prompt: string) => {
  try {
    const response = await fetch(`http://192.168.1.156:3000/api/justchat`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
